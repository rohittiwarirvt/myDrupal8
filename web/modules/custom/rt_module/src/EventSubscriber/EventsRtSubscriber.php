...
namespace Drupal\rt_module\EventSubscriber;
 ...
 */
class EventsRtSubscriber implements EventSubscriberInterface {
    ...
    public static function getSubscribedEvents() {
      ...
      $events[FeedsEvents::PARSE][] = ['notifyMario'];
      ...
      $events[FeedsEvents::PROCESS][] = ['notifyBatman', -100];
      $events[FeedsEvents::PROCESS_ENTITY_PRESAVE][] = ['notifyBatman_v1', -100];
      ...
      $events[FeedsEvents::PROCESS_ENTITY_PREVALIDATE][] = ['notifyDefault', -255];
     ...
     *   The event object containing the incident report.
     */
    public function notifyBatman(ProcessEvent $event) {
      #dsm($event,"processevnet");
      //    dsm($event->getItem(), "process_item");
      // if ($event->getItem()->get("certification_date")) {
      //   $original_value = $event->getItem()->get("certification_date");
      //   $csv_date = strtotime(trim($original_value) );
      //   if (empty($csv_date) && strpos($original_value, "/") ) {
      //     $data_value = explode("/", $original_value);
      //     list($date, $month, $year) = $data_value;
      //     $new_value =  join("/", array($month, $date, $year));
      //     $hard_code_new_value = "4/12/2020";
      //     $event->getItem()->set("certification_date", $hard_code_new_value);
      //   }
     ...
     *   The event object containing the incident report.
     */
    public function notifyBatman_v1(EntityEvent $entity) {
      #dsm($event,"processevnet");
      //  dsm($entity->getItem(), "entity_item");
      // if ($entity->getItem()->get("certification_date")) {
      //   $original_value = $entity->getItem()->get("certification_date");
      //   $csv_date = strtotime(trim($original_value) );
      //   if (empty($csv_date) && strpos($original_value, "/") ) {
      //     $data_value = explode("/", $original_value);
      //     list($date, $month, $year) = $data_value;
      //     $new_value =  join("/", array($month, $date, $year));
      //     $hard_code_new_value = "4/12/2020";
      //     $entity->getItem()->set("certification_date", $hard_code_new_value);
      //   }
      // }
      // //   if (strpos($original_value, "-")) {
      //      $data_value = explode("-", $original_value);
     ...
     *   The event object containing the incident report.
     */
    public function notifyDefault(EntityEvent $event) {
      #dsm($event,"EntityEvent");
